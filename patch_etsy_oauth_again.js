...
let blocks = code.split('// Etsy OAuth PKCE State');
if (blocks.length > 1) {
  let postBlocks = blocks[1].split('// Serve frontend static files');
  code = blocks[0] + newRoutes + '// Serve frontend static files' + postBlocks[1];
  fs.writeFileSync('server/index.js', code);
  console.log('Backend auth routes patched to allow custom callback URL.');
}
